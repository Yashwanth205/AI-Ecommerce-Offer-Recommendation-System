...
def edit_product(id):
    ...
    response = supabase.table("products2").select("*").eq("id", id).single().execute()
...
def delete_product(id):
    supabase.table("products2").delete().eq("id", id).execute()
    return redirect(url_for("products.view_products"))
...
def search_products():
    ...
    response = supabase.table("products2").select("*").ilike("name", f"%{query}%").execute()
